...
const Card = ({
  ...
  const toggle = () => setHovered((prev) => !prev);

  return (
    <div
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      onClick={toggle} // ← Tap to toggle on mobile
    ...
    >
      ...
      <div className="relative z-20 px-10">
        ...
        {/* Center Icon when inactive */}
        {!hovered && (
          <div
            className="text-center absolute top-[50%] left-[50%] 
          translate-x-[-50%] translate-y-[-50%] transition duration-200 
        ...
        {/* Title */}
        <h2
          className={`dark:text-white text-center text-3xl relative z-10 
          text-black mt-4 font-bold transition duration-300
        ...
        {/* Description */}
        <p
          className={`text-sm relative z-10 mt-4 text-center transition duration-300
          ${hovered ? "opacity-100 -translate-y-2 text-white" : "opacity-0"}`}
          style={{ color: hovered ? "white" : "#E4ECFF" }}
        >
          {des}
        </p>
      </div>
